...
class GearIndicatorLCD():
    ...
    """
    displayImage(filename, colOffset, rowOffset)



    """
    def displayImage(self, filename, colOffset, rowOffset):
        processedImage = self.LCDImage(filename)
        image_data = processedImage.data
    ...
    """
     displayGear(gear)
        ...
        gear - the gear which should be displayed on the LCD
            0 - Neutral
            1 - First Gear
            2 - Second Gear
            etc.

    This is a specialized function specifically for the car hud project.  Not really useful
    in any generic applications.  For generic image displaying use displayImage().
    """
    def displayGear(self, gear):
        ...
        try:
            ...
            if gear==0:
                image_data = self.neutralImage.data
            elif gear==1:
                image_data = self.firstGearImage.data
            elif gear==2:
                image_data = self.secondGearImage.data
            elif gear==3:
                image_data = self.thirdGearImage.data
            elif gear==4:
                image_data = self.fourthGearImage.data
            elif gear==5:
                image_data = self.fifthGearImage.data
            else:
